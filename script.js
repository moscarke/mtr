...
function changeCurrentTime (){
	...
	document.getElementById("currTime").innerHTML = currentTime.toLocaleTimeString('en-HK', {hourCycle: 'h23'});
}

function refreshSearch(station){
	let line, routeList = document.getElementsByClassName("mtrTable");
	for (let i = 0; i < routeList.length; i++){
		if (routeList[i].style.display == ""){
			line = routeList[i].id;
			break;
		}
	}
	...
	getCoords().then(coords => {
		if (coords.latitude != null){
			markdown("MTR REFRESH", coords.latitude, coords.longitude, coords.accuracy);
		} else {
			markdown("MTR REFRESH", "Rejected, permission state: " , coords,  "");
		}
	});
	schQuery(station, line, true);
}
